...
* [Example 1](https://stackoverflow.com/questions/71403633/conditional-formatting-of-cells-using-if-else-or-hiding-columns-in-dt-package-i) n 
...
<center> Remember, the goal is to recreate your problem! \n
Tailor your minimal data set to meet that goal. </center>
...
iris %>% head() %>% df_format() 
```
Then you an paste the data with ([CTRL + V] on many systems). 

Pull part of table into R-formatted syntax: 
```{r, eval=FALSE}
# first, copy part of a table from a spreadsheet
  #e.g. Googlesheets, Excel, Numbers, Calc
# next type these line in the console and run
tribble_paste() # to format your copied data as a tibble
df_paste() # to format your copied data as a data.frame
```
...
* Don't use screenshots when code will work
