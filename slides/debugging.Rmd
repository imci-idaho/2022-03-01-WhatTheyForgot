...
##### but, you will probably need to work with debugging functions eventually
...
For any R function, it will alert you when there is a problem.

This example examines `print()` when it receives output that is numeric and greater than or equal to 3. 
...
```{r}
log_hist <- function(var, color = "cyan", do_log=TRUE) {
  if(do_log == TRUE) {
    logvar = log(var)
    hist(logvar, col = color)
  }
  else hist(var, col = color)
}
```
